...
@section('content')
<style>
    .customer-dashboard { max-width: 1200px; margin: 0 auto; padding: 3rem 1.5rem; }
    .dashboard-welcome { background: linear-gradient(135deg, #1A1A1A, #34302a); border-left: 6px solid #F5A623; border-radius: 18px; color: #fff; padding: 2.25rem; margin-bottom: 1.75rem; }
    ...
    .dashboard-stat, .dashboard-card { background: #fff; border-radius: 14px; box-shadow: 0 6px 22px rgba(26,26,26,.08); }
    .dashboard-stat { padding: 1.35rem; border-top: 4px solid #F5A623; }
    .dashboard-stat p { color: #4A4A4A; margin: 0; font-size: .9rem; }
    .dashboard-stat strong { color: #1A1A1A; display: block; font-size: 1.8rem; margin-top: .4rem; }
    .dashboard-content { display: grid; grid-template-columns: 2fr 1fr; gap: 1.5rem; }
    .dashboard-card { overflow: hidden; }
    .dashboard-card-head { padding: 1.4rem 1.5rem; border-bottom: 1px solid #f0ede6; display: flex; align-items: center; justify-content: space-between; gap: 1rem; }
    .dashboard-card h2 { color: #1A1A1A; font-size: 1.4rem; margin: 0; }
    .dashboard-card-body { padding: 1.5rem; }
    .order-row { display: flex; justify-content: space-between; gap: 1rem; padding: 1rem 0; border-bottom: 1px solid #f0ede6; }
    .order-row:last-child { border-bottom: 0; }
    .order-row strong { color: #1A1A1A; }
    .order-meta { color: #777; font-size: .88rem; }
    ...
    .dashboard-action { background: #FFFDF5; border: 1px solid #f2dfbb; border-radius: 10px; color: #1A1A1A; display: block; font-weight: 600; padding: 1rem; transition: .2s ease; }
    .dashboard-action:hover { background: #FFD166; color: #1A1A1A; transform: translateY(-2px); }
    .dashboard-action span { color: #4A4A4A; display: block; font-size: .82rem; font-weight: 400; margin-top: .15rem; }
    .dashboard-link { color: #D98C00; font-weight: 600; font-size: .9rem; }
